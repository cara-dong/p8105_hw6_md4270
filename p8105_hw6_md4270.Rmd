...
The majority of cities have actual odds ratios for solving homicides comparing male victims to female victims smaller than 1. This means that crimes involving male victims tend to have smaller odds of getting resolved compared to those involving female victims, keeping all other variables fixed and taking victim age, sex and race into accounts. 

The confidence intervals for about half of the cities exclude 1, which suggests that there is generally a significant difference in odds ratio for resolution by victim sex.

Albuquerque, MN seems to have the highest odds ratio, and New York, NY has the lowest odds ratio.
...
The density plot of bootstrapped R-squared estimates shows a roughly unimodal, slightly left-skewed distribution that reaches its highest density when R-squared is around 0.92.
...
The density plot of bootstrapped log(beta1 times beta2) estimates shows an unimodal, heavily left-skewed distribution that reaches its highest density when log(beta1 times beta2) is around -5.5.
...
The density plot of bootstrapped log(beta0 times beta1) estimates shows a roughly normally distribution. It is unimodal and reaches its highest density when log(beta0 times beta1) is around 2.1.
...
To propose and fit a model, we will first examine the correlation between each pairs of variables using a correlation matrix:
```{r message=FALSE, warning=FALSE}
...
Based on the correlation matrix, we want to exclude some variables from highly correlated pairs (correlation >=0.7) to avoid multicollinearity. The pairs are (ppbmi delwt),  (ppwt	delwt), (ppwt	ppbmi), (mrace	frace). 
...
Given the three variables' correlation with outcome bwt, we will only keep the one with highest aboslute correlation, which is delwt.
...
Similarly, given the two variables' correlation with outcome bwt, we will only keep the one with higher absolute correlation, which is cor_mrace.
...
Based on the violin plot, our proposed model with more predictors has the lowest range of distribution of Root Mean Square Error (RMSE). Model1 with only birth length and gestational age as predictors has the highest range of distribution of RMSE values.
